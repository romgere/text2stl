...
export default class TextMakerService extends Service {
  ...
  private glyphToShapes(
    ...
    // see https://stackoverflow.com/questions/3742479/how-to-cut-a-hole-in-an-svg-rectangle#comment72845893_11878784
    // https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/fill-rule
    // https://github.com/mrdoob/three.js/issues/16950
    // Voir : http://paperjs.org/reference/pathitem/#create-pathData
    // tools :
    // https://opentype.js.org/glyph-inspector.html
    // https://yqnn.github.io/svg-path-editor/

    // Try to play with something like ;
    // const p1 = new paper.Path(path)
    // const p2 = new paper.Path(hole)
    // p1.fillRule = 'evenodd'
    // p2.fillRule = 'evenodd'
    // p1.subtract(p2).pathData
    // p1.exclude(p2).pathData
    // p1.subtract(p2, { trace: false }).pathData
    // p1.exclude(p2, { trace: false }).pathData

    ;
    const pathData = glyphPath.map((p) => `${p.type}${p.values.join(' ')}`).join('');
    const test = paper.PathItem.create(pathData);
    console.log('pathData', pathData);

    const toto = new paper.CompoundPath(pathData);
    debugger;
