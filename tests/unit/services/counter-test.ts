...
module('Unit | Service | counter', function (hooks) {
  ...
  test('it exists', async function (assert) {
    ...
    assert.equal(service.counter, 0, 'first get for counter return 0');

    await service._initPromise;
    assert.equal(service.counter, 123, 'counter is now conform to counterApi response');

    expectedUrl = 'https://api.countapi.xyz/hit/text2stl/test_stl';
    fetchReturn = { value: 1256 };
    await service.updateCounter();

    assert.equal(service.counter, 1256, 'counter is conform to counterApi response');
