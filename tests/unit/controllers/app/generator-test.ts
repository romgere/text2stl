...
module('Unit | Controller | app/generator', function(hooks) {
  ...
  test('it handles font change & font loading (google font)', async function(assert) {
    ...
    this.owner.lookup('service:font-manager').fetchFont = async function(name: string, variant: Variant) {
    ...
    assert.equal(controller.font.value?.names.fontFamily['en'], 'mocked_my-font_500italic', 'Font was load on model')
  })
  ...
  test('it handles font change & font loading (custom font)', async function(assert) {
    ...
    // Wait for the font to be load
    await waitUntil(() => controller.font.isResolved)
    assert.equal(controller.font.value?.names.fontFamily['en'], 'mocked_my-font.file_100', 'Font was load on model')
  })
  ...
  test('it generate a STL with mesh', async function(assert) {
    ...
    this.owner.lookup('service:text-maker').generateMesh = function(settings: TextMakerParameters, font: Font) {
    ...
    assert.verifySteps([
      'gtag_event_stl_generation_2',
      'gtag_event_stl_download_2'
    ])
  })
})
