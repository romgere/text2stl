...
module('Integration | Component | ui/type-select', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(
      hbs`<Ui::TypeSelect @value={{this.current}} @onChange={{this.onChange}} data-custom-attrs="hello" />`,
    );
    ...
    assert
      .dom('input[type="radio"]:not(:checked)')
      .exists({ count: 3 }, 'There is 3 selected options');
